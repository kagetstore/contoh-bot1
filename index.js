...
╭❲ SELAMAT DATANG KANG SHARE LINK GRUP INTRO DULU NGAB ❳
│${mdata.subject}
│
│❀ *ɴᴀᴍᴀ* :
│❀ *ᴜᴍᴜʀ* :
...
╰────────────────⊱`
				...
				try {
					...
					teks = `ALHAMDULILLAH BEBAN KELUARGA KELUAR\n
            ...
            //Saya Mohon Jangan hapus list update gue ya
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    wew = fs.readFileSync(`./src/logobot.jpg`)
                      lzain = `
╭──❲ INFO BOT ❳
│❀ Nama: KAGETSTORE-BOTZ
│❀ Owner : KAGETSTORE GANZ
│❀ Prefix: 「  ${prefix}  」
╰────────────────⊱
╭──>
╰────────────────⊱
...
╭──❲ DEVELOPER ❳
│❀ SC ORY : MhanlBarBar
│❀ UPDATE SC : Riyan BOTZ
│❀ OWNER : https://wa.me/6285763500823
│❀ SUBREK YT : Riyan BOTZ
...
break
		...
		case 'lpr':
					...
					reply('Masalah telah di laporkan ke owner Riyan Laporan palsu atau main² tidak akan ditanggapi.')
					break
					case 'antidelete':
		...
		case 'request':
					...
					client.sendMessage('6283153406515@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6283153406515@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER RIYAN REQUESTS PALSU ATAU MAIN² TIDAK AKAN DITANGGAPI.')
					break
		case 'blocklist': 
...
if (args.length == 0) return reply(`Idnya mana kak?`)
		...
		case 'kick':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `╭────────────────⊱
║> *_SELAMAT TINGGAL ANAK HARAM.KICK_*
╰────────────────⊱
 🤭 :\n`
							teks += `@_.split('@')[0]`
						}
						...
						mentions(`╭────────────────⊱
║> *_MAMPUS KENA JUMSHOT MAKANNYA JANGAN SHARE LINK_*
╰────────────────⊱
